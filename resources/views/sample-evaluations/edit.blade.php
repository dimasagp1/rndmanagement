<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-2 text-sm text-gray-500">
            ...
            <span class="text-ink font-medium">Edit {{ $sampleEvaluation->sample_id }}</span>
        </div>
    </x-slot>

    <div class="page-header">
        <div>
            <h1 class="page-title">Edit {{ $sampleEvaluation->sample_id }}</h1>
            <p class="page-subtitle">Perbarui informasi header evaluasi sampel</p>
        </div>
    </div>

    <div class="card max-w-2xl">
        <form method="POST" action="{{ route('sample-evaluations.update', $sampleEvaluation) }}" class="p-6 space-y-5">
            @csrf
            @method('PUT')

            <div>
                <label for="sample_id" class="block text-sm font-medium text-gray-700 mb-1">Sample ID <span class="text-red-500">*</span></label>
                <input type="text" id="sample_id" name="sample_id" value="{{ old('sample_id', $sampleEvaluation->sample_id) }}"
            ...
            <div>
                ...
                <select id="project_owner_id" name="project_owner_id" class="form-input @error('project_owner_id') border-red-400 @enderror" required>
                    ...
                    <option value="{{ $user->id }}" {{ old('project_owner_id', $sampleEvaluation->project_owner_id) == $user->id ? 'selected' : '' }}>
                        {{ $user->name }}
                    </option>
                    @endforeach
                </select>
            ...
            <div class="flex items-center gap-2 pt-2">
                ...
                <a href="{{ route('sample-evaluations.show', $sampleEvaluation) }}" class="btn-ghost">Batal</a>
            </div>
        </form>
    </div>
</x-app-layout>