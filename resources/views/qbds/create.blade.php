<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-2 text-sm text-gray-500">
            ...
            <span class="text-ink font-medium">Tambah QbD</span>
        </div>
    </x-slot>

    <div class="min-h-screen max-w-3xl">
        <header class="mb-8">
            <h1 class="text-2xl font-heading font-bold text-ink mb-1">Tambah QbD</h1>
            <p class="text-sm text-gray-500">
                Isi nama produk dan unggah lampiran dokumen (PDF/Word/Image, opsional).
            </p>
        </header>

        <form method="POST" action="{{ route('qbds.store') }}" enctype="multipart/form-data" class="space-y-6">
            @csrf
