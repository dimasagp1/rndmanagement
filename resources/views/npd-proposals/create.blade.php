<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-2 text-sm text-gray-500">
            ...
            <span class="text-gray-400">Buat Proposal Baru</span>
        </div>
    </x-slot>
    ...
    <div class="min-h-screen max-w-3xl">
        {{-- ─── Header ─────────────────────────────────────── --}}
        <header class="mb-8">
            <h1 class="text-2xl font-heading font-bold text-ink mb-1">Buat NPD Proposal Baru</h1>
            <p class="text-sm text-gray-500">
                Dokumen permintaan pengembangan produk yang menjadi dasar resmi dimulainya proyek NPD.
            </p>
        </header>

        <form method="POST" action="{{ route('npd-proposals.store') }}" enctype="multipart/form-data"
              class="space-y-6" x-data="proposalForm()">
            @csrf
            ...
            <div class="card card-body space-y-4">
                ...
                <div>
                    <label for="prf_id" class="block text-sm font-medium text-gray-700 mb-1">PRF (Submitted) *</label>
                    <select id="prf_id" name="prf_id" x-model="prfId"
                            class="w-full rounded-lg border-gray-300 bg-gray-50 px-4 py-2.5 text-sm focus:border-primary focus:ring-primary">
                        <option value="">— Pilih PRF yang sudah disetujui —</option>
                        @foreach($prfs as $prf)
                        <option value="{{ $prf->id }}" {{ old('prf_id') == $prf->id ? 'selected' : '' }}
                            data-name="{{ $prf->product_name }}" data-concept="{{ $prf->product_concept }}">
                            {{ $prf->code }} — {{ $prf->product_name ?: 'Tanpa nama produk' }}
                        </option>
                        @endforeach
                    </select>
                    @error('prf_id') <p class="text-xs text-red-600 mt-1">{{ $message }}</p> @enderror
                    <p class="text-xs text-gray-400 mt-1">Nama produk & konsep otomatis terisi dari PRF yang dipilih.</p>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select id="pic" name="pic"
                            ...
                            <option value="{{ $member->name }}" {{ old('pic', auth()->user()->name) == $member->name ? 'selected' : '' }}>
                                {{ $member->name }}
                            </option>
                            @endforeach
                            @if(old('pic') && ! $teamMembers->pluck('name')->contains(old('pic')))
                            <option value="{{ old('pic') }}" selected>{{ old('pic') }} (PIC lama)</option>
                            @endif
                        </select>
                        @error('pic') <p class="text-xs text-red-600 mt-1">{{ $message }}</p> @enderror
                        <p class="text-xs text-gray-400 mt-1">Daftar PIC diambil dari akun pengguna (Staff R&D / Staff Packdev).</p>
                    </div>
                </div>

                <div>
                    <label for="product_concept" class="block text-sm font-medium text-gray-700 mb-1">Product Concept *</label>
                    <textarea id="product_concept" name="product_concept" x-model="productConcept" rows="4"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" id="target_cogs" name="target_cogs" step="0.01" min="0" value="{{ old('target_cogs') }}"
                               class="w-full rounded-lg border-gray-300 bg-gray-50 px-4 py-2.5 text-sm focus:border-primary focus:ring-primary"
                    ...
                    <div>
                        ...
                        <input type="number" id="target_selling_price" name="target_selling_price" step="0.01" min="0" value="{{ old('target_selling_price') }}"
                               class="w-full rounded-lg border-gray-300 bg-gray-50 px-4 py-2.5 text-sm focus:border-primary focus:ring-primary"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="date" id="development_start" name="development_start" value="{{ old('development_start') }}"
                               class="w-full rounded-lg border-gray-300 bg-gray-50 px-4 py-2.5 text-sm focus:border-primary focus:ring-primary">
                    ...
                    <div>
                        ...
                        <input type="date" id="development_end" name="development_end" value="{{ old('development_end') }}"
                               class="w-full rounded-lg border-gray-300 bg-gray-50 px-4 py-2.5 text-sm focus:border-primary focus:ring-primary">
                ...
                <div>
                    ...
                    <textarea id="project_team" name="project_team" rows="3"
                              class="w-full rounded-lg border-gray-300 bg-gray-50 px-4 py-2.5 text-sm focus:border-primary focus:ring-primary"
                              placeholder="cth: Ahmad Fauzi (Formulasi), Budi (Packdev), Citra (QA)...">{{ old('project_team') }}</textarea>
            ...
            <div class="card card-body">
                ...
                <div id="documents-empty"
                     class="flex flex-col items-center justify-center py-10 px-4 text-center border-2 border-dashed border-gray-200 rounded-xl bg-gray-50/50">
                    <svg class="w-10 h-10 text-gray-300 mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"/>
                    </svg>
                    <p class="text-sm font-semibold text-gray-500">Belum ada file yang dipilih</p>
                    <p class="text-xs text-gray-400 mt-1">
                        Klik <span class="font-semibold text-emerald-600">"Tambah Dokumen"</span> untuk memilih file pendukung NPD Proposal Anda.
                    </p>
                </div>
    ...
    <script>
        function proposalForm() {
            return {
                prfId: '{{ old('prf_id', '') }}',
                productName: '{{ addslashes(old('product_name', '')) }}',
                productConcept: '{{ addslashes(old('product_concept', '')) }}',
                init() {
                    this.$watch('prfId', (id) => {
                        if (!id) return;
                        const opt = document.querySelector(`#prf_id option[value="${id}"]`);
                        if (opt) {
