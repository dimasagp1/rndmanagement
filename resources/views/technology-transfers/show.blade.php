<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-2 text-sm text-gray-500 flex-wrap">
            ...
            <span class="text-ink font-medium truncate max-w-[300px]">{{ $technologyTransfer->title }}</span>
        </div>
    </x-slot>

    <div class="page-header flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div class="min-w-0">
            <h1 class="page-title">{{ $technologyTransfer->title }}</h1>
        ...
        <div class="flex items-center gap-3 flex-wrap">
            ...
            @endcan
            <a href="{{ route('technology-transfers.index') }}" class="btn-ghost">← Kembali</a>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="card">
                <div class="card-header">
                    <h2 class="card-title">Informasi Transfer</h2>
                </div>
                <div class="card-body space-y-4">
                    <div>
                        <p class="text-xs text-gray-400">Dibuat Oleh</p>
                        <p class="font-medium text-ink">{{ $technologyTransfer->creator?->name ?? '—' }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-400">Tanggal Dibuat</p>
                        <p class="font-medium text-ink">{{ $technologyTransfer->created_at?->format('d M Y H:i') ?? '—' }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-400">Terakhir Diperbarui</p>
                        <p class="font-medium text-ink">{{ $technologyTransfer->updated_at?->format('d M Y H:i') ?? '—' }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    @if($technologyTransfer->attachments->isEmpty())
        ...
        <div class="lg:col-span-1">
            <div class="card sticky top-4">
                ...
                <div class="card-body">
                    @if($technologyTransfer->attachments->where('is_previewable', true)->first())
                        @foreach($technologyTransfer->attachments->where('is_previewable', true) as $attachment)
                        <div class="mb-4">
    ...
    <div id="uploadAttachmentModal" class="hidden fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
        <div class="bg-white rounded-2xl shadow-xl p-6 w-full max-w-md">
            ...
            <form method="POST" action="{{ route('technology-transfers.attachments.store', $technologyTransfer) }}" enctype="multipart/form-data" class="mt-4">
                @csrf
