<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-2 text-sm text-gray-500">
            <a href="{{ route('approval-formula-designs.index', ['type' => $type]) }}" class="hover:text-primary">{{ $type === 'Design' ? 'Design Approval' : 'Approval ' . $type }}</a>
    ...
    <div class="min-h-screen max-w-3xl">
        <header class="mb-8">
            <h1 class="text-2xl font-heading font-bold text-ink mb-1">Tambah {{ $type === 'Design' ? 'Design Approval' : 'Approval ' . $type }}</h1>
        </header>
