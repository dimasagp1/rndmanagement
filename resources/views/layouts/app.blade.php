        ...
        >
            ...
            <nav class="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
                ...
                <a href="{{ route('timeline.index') }}"
                   class="sidebar-link {{ request()->routeIs('timeline.*') || request()->routeIs('dashboard') ? 'active' : '' }}">
                    <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/>
                    </svg>
                ...
                @php($modulePerms = ['prf' => 'prf.view', 'npd-proposal' => 'npd_proposal.view', 'preformulation-qbd' => 'qbd.view', 'formulation-development' => 'formula.view', 'sample-evaluation' => 'sample_evaluation.view', 'formula-approval' => 'formula.view', 'stability-test' => 'stability_test.view', 'approval-formula-design' => 'formula.view', 'regulatory-dossier' => 'regulatory_dossier.view', 'technology-transfer' => 'technology_transfer.view', 'nie-approved' => 'nie_approval.view', 'commercial-production' => 'commercial_production.view'])
                @php($moduleRoutes = ['prf' => 'prfs.index', 'npd-proposal' => 'npd-proposals.index', 'preformulation-qbd' => 'qbds.index', 'formulation-development' => 'formulas.index', 'sample-evaluation' => 'sample-evaluations.index', 'formula-approval' => 'formula-approvals.index', 'stability-test' => 'stability-tests.index', 'approval-formula-design' => 'approval-formula-designs.index', 'regulatory-dossier' => 'regulatory-dossier.index', 'technology-transfer' => 'technology-transfers.index', 'nie-approved' => 'nie-approvals.index', 'commercial-production' => 'commercial-productions.index'])
                @php($moduleActive = collect($moduleRoutes)->filter(fn($r) => request()->routeIs(str($r)->before('.') . '.*'))->keys()->all())
                @php($moduleActive = array_merge($moduleActive, request()->routeIs('approval-formula-designs.*') ? ['approval-formula-design'] : []))
                ...
                <div x-data="{ open: {{ (in_array(request()->route('tab'), $slugs) || array_intersect($moduleActive, $slugs)) ? 'true' : 'false' }} }">
                    ...
                    <div x-show="open" x-transition:enter="transition ease-out duration-150"
                        ...
                        @if($slug === 'formula-approval')
                        <a href="{{ route('formula-approvals.index') }}"
                           class="sidebar-sub-link {{ request()->routeIs('formula-approvals.*') && !request('type') ? 'active' : '' }}">
                            <span class="flex-1 truncate">{{ $general::TABS[$slug] }}</span>
                        </a>
                        @continue
                        @endif
                ...
                <a href="{{ route('product-categories.index') }}"
                   class="sidebar-link {{ request()->routeIs('product-categories.*') ? 'active' : '' }}">
                    <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" class="w-5 h-5 flex-shrink-0">
