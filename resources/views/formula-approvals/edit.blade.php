<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-2 text-sm text-gray-500 min-w-0">
            <a href="{{ route('formula-approvals.index') }}" class="hover:text-primary transition shrink-0">Approval Formula & Design</a>
    ...
    <div class="min-h-screen max-w-3xl">
        <header class="mb-8">
            <h1 class="text-2xl font-heading font-bold text-ink mb-1">Edit — Approval Formula & Design</h1>
        ...
        <form method="POST" action="{{ route('formula-approvals.update', $formApproval) }}" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="card card-body space-y-5">
                @include('formula-approvals.partials.form-fields', ['form' => $formApproval, 'categories' => $categories, 'products' => $products, 'formulas' => $formulas])
            </div>
