<x-app-layout>
    ...
    <div class="max-w-[1200px] mx-auto">
        <!-- Top Action Bar: Search | Export | Add File | More -->
            ...
            <div class="flex items-center gap-2 ml-auto">
                ...
                <div class="relative" x-data="{ openExport: false }">
                    ...
                    <div x-show="openExport" @click.outside="openExport = false" x-transition class="absolute right-0 mt-2 w-64 bg-white border border-gray-200 rounded-xl shadow-xl py-1.5 z-50 text-xs">
                        <a href="{{ route('commercial-productions.export.zip', array_merge(request()->only('search','type'), $currentFolder ? ['folder' => $currentFolder->id] : [])) }}" class="px-4 py-2.5 text-gray-700 hover:bg-gray-50 flex items-start gap-2.5 transition">
                            ...
                            <div>
                                ...
                                <p class="text-[11px] text-gray-400">Unduh seluruh berkas asli {{ $currentFolder ? 'dalam folder ini & sub-foldernya' : 'semua berkas commercial production' }}</p>
                            </div>
                        </a>
                        <div class="border-t border-gray-100 my-1"></div>
                        <a href="{{ route('commercial-productions.export.excel', array_merge(request()->only('search','type'), $currentFolder ? ['folder' => $currentFolder->id] : [])) }}" class="px-4 py-2.5 text-gray-700 hover:bg-gray-50 flex items-start gap-2.5 transition">
                ...
                <div class="relative" x-data="{ open: false }">
                    ...
                    <div x-show="open" @click.outside="open = false" x-transition class="absolute right-0 mt-2 w-52 bg-white border border-gray-200 rounded-xl shadow-lg py-1 z-30 text-xs">
                        ...
                        @if($currentFolder)
                            @can('commercial_production.edit')
                            ...
                            @endcan
                            @can('commercial_production.delete')
                            <form method="POST" action="{{ route('commercial-productions.folders.destroy', $currentFolder) }}" onsubmit="return confirm('Apakah Anda yakin ingin menghapus folder ini beserta seluruh isinya? Tindakan ini tidak dapat dibatalkan.')">
                                @csrf
                                @method('DELETE')
                            ...
                            @endcan
                            <div class="border-t border-gray-100 my-1"></div>
                            <a href="{{ route('commercial-productions.export.zip', ['folder' => $currentFolder->id]) }}" class="w-full text-left px-4 py-2.5 hover:bg-gray-50 flex items-center gap-2 text-gray-700">
                            ...
                            <a href="{{ route('commercial-productions.export.excel', ['folder' => $currentFolder->id]) }}" class="w-full text-left px-4 py-2.5 hover:bg-gray-50 flex items-center gap-2 text-gray-700">
                    ...
                    @foreach($folders as $index => $folder)
                        <div x-data="{ open: false }"
                            ...
                            <div class="flex items-start justify-between">
                                <a href="{{ route('commercial-productions.index', ['folder' => $folder->id]) }}" class="w-10 h-10 rounded-xl flex items-center justify-center transition hover:scale-105" style="background-color: #F8F8F6;">
                                ...
                                <div class="relative">
                                    ...
                                    <div x-show="open" @click.outside="open = false" x-transition class="absolute right-0 mt-1 w-44 bg-white border border-gray-200 rounded-xl shadow-xl py-1 z-50 text-xs">
                                        <a href="{{ route('commercial-productions.index', ['folder' => $folder->id]) }}" class="px-3 py-2 text-gray-700 hover:bg-gray-50 flex items-center gap-2">
                                            <svg class="w-3.5 h-3.5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                            Buka Folder
                                        </a>
                                        <a href="{{ route('commercial-productions.export.zip', ['folder' => $folder->id]) }}" class="px-3 py-2 text-gray-700 hover:bg-gray-50 flex items-center gap-2">
                                        ...
                                        @can('commercial_production.delete')
                                        <div class="border-t border-gray-100 my-1"></div>
                                        <form method="POST" action="{{ route('commercial-productions.folders.destroy', $folder) }}" onsubmit="return confirm('Apakah Anda yakin ingin menghapus folder \'{{ addslashes($folder->name) }}\' beserta seluruh sub-folder dan file di dalamnya? Tindakan ini tidak dapat dibatalkan.')">
                                            @csrf
                                            @method('DELETE')
                            ...
                            <a href="{{ route('commercial-productions.index', ['folder' => $folder->id]) }}" class="mt-4 block flex-1 flex flex-col justify-end">
                            ...
                            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 p-4">
                                @foreach($documents as $doc)
                                    <div x-data="{ open: false }"
                                        ...
                                        <div class="flex items-start justify-between mb-2">
                                            ...
                                            <div class="relative">
                                                ...
                                                <div x-show="open" @click.outside="open = false" x-transition class="absolute right-0 mt-1 w-40 bg-white border border-gray-200 rounded-xl shadow-xl py-1 z-50 text-xs">
                                                    <a href="{{ route('commercial-productions.documents.show', $doc) }}" class="px-3 py-2 text-gray-700 hover:bg-gray-50 flex items-center gap-2">
                                                        <svg class="w-3.5 h-3.5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                                        Detail File
                                                    </a>
                                                    <a href="{{ route('commercial-productions.documents.download', $doc) }}" class="px-3 py-2 text-gray-700 hover:bg-gray-50 flex items-center gap-2">
                                                        <svg class="w-3.5 h-3.5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
                                                        Download
                                                    </a>
                                                    @can('commercial_production.edit')
                                                    ...
                                                    @can('commercial_production.delete')
                                                    <div class="border-t border-gray-100 my-1"></div>
                                                    <form method="POST" action="{{ route('commercial-productions.documents.destroy', $doc) }}" onsubmit="return confirm('Apakah Anda yakin ingin menghapus file \'{{ addslashes($doc->original_name) }}\'?')">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="w-full text-left px-3 py-2 text-red-600 hover:bg-red-50 flex items-center gap-2">
                                                            <svg class="w-3.5 h-3.5 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                                        ...
                                        <a href="{{ route('commercial-productions.documents.show', $doc) }}" class="text-sm font-medium hover:text-primary line-clamp-2 leading-tight block" style="color: #1F2A22;" title="{{ $doc->original_name }}">{{ $doc->original_name }}</a>
                            ...
                            <div class="divide-y divide-gray-50">
                                @foreach($documents as $doc)
                                    <div class="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 transition group">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            <a href="{{ route('commercial-productions.documents.show', $doc) }}" class="text-sm font-medium hover:text-primary truncate block" style="color: #1F2A22;" title="{{ $doc->original_name }}">{{ $doc->original_name }}</a>
                                        ...
                                        <div class="flex items-center gap-1">
                                            <a href="{{ route('commercial-productions.documents.download', $doc) }}" class="w-8 h-8 rounded-lg bg-white border border-gray-200 flex items-center justify-center hover:bg-gray-50 text-gray-500 hover:text-ink transition" title="Download">
    ...
    <script>
        ...
        function openRenameFolderModal(folderId, name, desc) {
            ...
            form.action = '{{ url("commercial-productions/folders") }}/' + folderId;
        ...
        function openMoveModal(docId, docName, currentFolderId) {
            ...
            form.action = '{{ url("commercial-productions/documents") }}/' + docId + '/move';
